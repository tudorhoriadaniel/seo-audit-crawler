...
async function clusterGscKeywords(client, { siteUrl, rows }) {
  ...
  if (client) {
    try {
      ...
      fallbackReason = 'AI clustering failed (' + e.message + ')';
    }
  } else {
    fallbackReason = 'no AI key configured on the server';
  }
  ...
  return { source, fallbackReason, clusters, keywordCount: queries.length, truncated: byQuery.size > queries.length };
}
...
function heuristicGscClusters(queries) {
  ...
  for (const [url, kws] of byUrl) {
    ...
    clusters.push({
      ...
      rationale: 'Grouped by strongest ranking URL (heuristic fallback).'
    });
  }
  return { clusters };
}
...
async function buildContentStrategy(client, { siteUrl, pages }) {
  ...
  if (client) {
    try {
      ...
      fallbackReason = 'AI analysis failed (' + e.message + ')';
    }
  } else {
    fallbackReason = 'no AI key configured on the server';
  }
