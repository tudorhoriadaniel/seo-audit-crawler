...
class Analyzer {
  ...
  _hreflangReport() {
    ...
    // Keep two maps: exact, and a normalised key (lowercase host, no
    // query/fragment, no trailing slash) used as a fallback. Also index
    // finalUrl so redirect targets resolve.
    const norm = (u) => {
      try {
        const x = new URL(u);
    ...
    for (const res of this.results) {
      ...
      statusByUrl.set(res.url, res.statusCode);
      if (res.finalUrl) statusByUrl.set(res.finalUrl, res.statusCode);
      const nk = norm(res.url);
      if (!statusByNorm.has(nk)) statusByNorm.set(nk, res.statusCode);
    }
