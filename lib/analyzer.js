...
class Analyzer {
  ...
  _canonicalReport() {
    ...
    const pages = this.results.filter(r => r.statusCode >= 200 && r.statusCode < 300 && (!r.redirectChain || r.redirectChain.length === 0) && (!r.finalUrl || r.finalUrl === r.url));
  ...
  _metaTitlesReport() {
    const pages200 = this.results.filter(r => r.statusCode >= 200 && r.statusCode < 300);
