...
class Analyzer {
  ...
  _hreflangReport() {
    ...
    // each hreflang target with the HTTP status it returned (or null if
    // the target was never crawled, e.g. an external/uncrawled URL).
    const statusByUrl = new Map();
    for (const res of this.results) {
      if (res && res.url) statusByUrl.set(res.url, res.statusCode);
    }
    ...
    return {
      ...
      pagesList: pages.map(p => ({
        ...
        hreflangs: (p.hreflangs || []).map(h => ({
          ...
          href: h.href,
          isSelf: h.href === p.url,
          status: statusByUrl.has(h.href) ? statusByUrl.get(h.href) : null
        }))
      }))
    };
  }
