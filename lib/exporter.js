...
class Exporter {
  ...
  static toXLSX(pages, analysis) {
    ...
    if (pagesData.length > 0) {
      const cols = Object.keys(pagesData[0]).map(k => {
        const maxLen = Math.max(k.length, ...pagesData.slice(0, 100).map(r => String(r[k] || '').length));
    ...
    // Issues grouped by category – one sheet per category
    if (analysis && analysis.issues && analysis.issues.length > 0) {
      ...
      for (const i of analysis.issues) {
        ...
        byCategory[cat].push({
          'URL': i.url,
          'Issue': i.message || i.issue || '',
          'Severity': i.severity,
          'Type': i.type || ''
        });
      }
      // Sort categories alphabetically for consistent sheet order
      ...
      for (const cat of categories) {
        ...
        const sheetName = cat.replace(/[\\/*?\[\]:]/g, '').substring(0, 31) || 'Other';
    ...
    // Marketing parameter preservation on redirects
    if (analysis && analysis.redirectParamsReport) {
      ...
      if (paramData.length > 0) {
        const ws5 = XLSX.utils.json_to_sheet(paramData);
        XLSX.utils.book_append_sheet(wb, ws5, 'Redirect Param Loss');
      }
    }

    // WAF / bot-protection challenges
    if (analysis && analysis.botAccessReport && analysis.botAccessReport.challengedCount > 0) {
      const rows = analysis.botAccessReport.pages.map(p => ({
        'URL': p.url, 'Status': p.statusCode, 'WAF Vendor': p.vendor
      }));
      const wsWaf = XLSX.utils.json_to_sheet(rows);
      XLSX.utils.book_append_sheet(wb, wsWaf, 'WAF Challenges');
    }

    // Cloaking / UA-parity check
    if (analysis && analysis.botParityReport && analysis.botParityReport.tested) {
      const bp = analysis.botParityReport;
      const rows = (bp.results || []).map(r => ({
        'URL': r.url,
        'Result': r.error ? `Error: ${r.error}` : (r.challenge ? `WAF challenge (${r.challenge})` : (r.differs ? 'DIFFERS' : 'Match')),
        'Differs In': r.diffs.map(d => d.field).join(', '),
        'Details': r.diffs.map(d => `${d.field}: "${d.crawled}" vs "${d.compared}"`).join(' | '),
        'Crawled As': bp.crawledAs,
        'Compared With': bp.comparedWith
      }));
      if (rows.length > 0) {
        const wsClk = XLSX.utils.json_to_sheet(rows);
        XLSX.utils.book_append_sheet(wb, wsClk, 'Cloaking Check');
      }
    }
