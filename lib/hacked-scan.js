...
// decodeURIComponent throws URIError("URI malformed") on invalid %-sequences,
// which real-world spam links contain all the time — fall back to the raw string.
...
function findSpamLinks($, pageHost) {
  ...
  $('a[href^="http"]').each((_, a) => {
    ...
    const { score, hits } = spamScore(anchor + ' ' + safeDecode(u.pathname + u.search).replace(/[-_/+]/g, ' ') + ' ' + u.host.replace(/[-.]/g, ' '));
...
async function scanForHack(siteInput, options = {}) {
  ...
  const htmlSampleSize = Math.min(Math.max(parseInt(options.htmlSample, 10) || 12, 3), 100);
  ...
  // Each page is wrapped: one pathological page (broken markup, hostile URLs)
  // must degrade coverage, never abort the whole scan.
