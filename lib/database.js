...
class CrawlDatabase {
  ...
  // Pages within a few clicks of the homepage, with just the columns the
  // content-strategy builder needs (incl. the links blob for inbound-link
  // counting). Depth-bounded so huge crawls don't ship megabytes of links.
  getPagesForContentStrategy(id, maxDepth) {
    return this.db.prepare(`
      SELECT url, final_url, status_code, content_type, meta_robots, x_robots_tag,
             title, meta_description, h1, heading_structure, word_count, depth,
             links, canonical, canonical_is_self, error, blocked_by_robots
      FROM pages WHERE crawl_id = ? AND depth <= ?
      ORDER BY depth ASC, url ASC
    `).all(id, maxDepth);
  }
