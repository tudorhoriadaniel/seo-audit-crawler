...
function generatePDFReport(res, analysis, siteUrl) {
  ...
  function ensureSpace(need) {
    if (y + need > 770) { doc.addPage(); y = 50; }
  }
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    if (i > 0) {
      doc.text(siteUrl + ' | SEO Audit Report', LM, 805, { width: PW * 0.6, align: 'left' });
    }
    doc.text(`Page ${i + 1} of ${range.count}`, LM, 805, { width: PW, align: 'right' });
  }
