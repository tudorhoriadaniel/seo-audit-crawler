...
function generatePDFReport(res, analysis, siteUrl) {
  ...
  function addFooter() {
    ...
    doc.text(siteUrl + ' | SEO Audit Report', LM, 800, { width: PW * 0.7, align: 'left', lineBreak: false });
    doc.text('Page ' + pageNum, LM, 800, { width: PW, align: 'right', lineBreak: false });
    doc.restore();
  }
  ...
  function heading(text) {
    ...
    doc.fill(C.white).fontSize(13).font('Helvetica-Bold').text(text, LM + 12, y + 7, { width: PW - 24 });
    y += 36;
    doc.fill(C.text);
  }
  ...
  function table(headers, rows, widths) {
    ...
    // Rows
    rows.forEach((row, ri) => {
      if (y > 755) { addFooter(); doc.addPage(); pageNum++; y = 50; }
  ...
  if (sm.found) {
    ...
    para(`URLs in sitemap: ${sm.totalSitemapUrls || 0} | Crawled not in sitemap: ${sm.crawledNotInSitemapCount || 0} | In sitemap not crawled: ${sm.inSitemapNotCrawledCount || 0}`);
