...
function generatePDFReport(res, analysis, siteUrl) {
  ...
  function table(headers, rows, widths) {
    ...
    rows.forEach((row, ri) => {
      ...
      let rowH = 16;
      row.forEach((cell, i) => {
        const txt = String(cell || '-').substring(0, 200);
        const h = doc.heightOfString(txt, { width: widths[i] - 6 });
        rowH = Math.max(rowH, h + 6);
      });
      rowH = Math.min(rowH, 60); // cap at 60px

      if (y + rowH > 740) { addFooter(); doc.addPage(); pageNum++; y = 50; }
      if (ri % 2 === 1) doc.rect(LM, y, PW, rowH).fill('#f8fafc');
      x = LM;
      row.forEach((cell, i) => {
        doc.fill(C.text).fontSize(7).font('Helvetica').text(String(cell || '-').substring(0, 200), x + 3, y + 3, { width: widths[i] - 6, height: rowH - 4, ellipsis: true });
        x += widths[i];
      });
      y += rowH;
    });
  ...
  if ((md.duplicates?.length || 0) > 0) {
    ...
    table(['Duplicate Description', 'Count', 'URLs'], md.duplicates.slice(0, 10).map(d => [d.description, d.count + 'x', d.urls.slice(0, 3).join(', ')]), [200, 35, PW - 235]);
  }
