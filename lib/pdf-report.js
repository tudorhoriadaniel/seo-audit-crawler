...
function generatePDFReport(res, analysis, siteUrl) {
  ...
  if ((bar.challengedCount || 0) > 0) {
    tip('ISSUE', `${bar.challengedCount} request(s) were challenged by bot protection (${Object.keys(bar.byVendor || {}).join(', ')}) while crawling as ${bar.botLabel || 'the default UA'}. Data for those URLs reflects the WAF's challenge page, not the site. If this is your own site, allowlist the SEOAuditCrawler user agent and re-crawl.`, '#fee2e2');
