...
function generatePDFReport(res, analysis, siteUrl) {
  ...
  // Decode URL-looking cells so diacritics render as letters (…/qualité) like
  // Google Search Console, not …/qualit%C3%A9. Only touches strings that hold
  // a URL at line start, so titles / descriptions are left intact.
  const _decUrl = (s) => { try { return decodeURI(s); } catch { return s; } };
  const _decCell = (v) => (typeof v === 'string' && /(^|\n)\s*(https?:)?\/\//.test(v)) ? _decUrl(v) : v;

  function table(headers, rows, widths) {
    rows = (rows || []).map(row => Array.isArray(row) ? row.map(_decCell) : row);
    ensureSpace(28);
  ...
  // WAF / bot-protection challenges + cloaking check
  const bar = analysis.botAccessReport || {};
  const bpr = analysis.botParityReport || {};
  const bprDiffs = (bpr.results || []).filter(r => r.differs);
  if ((bar.challengedCount || 0) > 0) {
    tip('ISSUE', `${bar.challengedCount} request(s) were challenged by bot protection (${Object.keys(bar.byVendor || {}).join(', ')}) while crawling as ${bar.botLabel || 'the default UA'}.${bar.challengeAborted ? ` The crawl was stopped early after ${bar.challengeAbortAfter} pages because essentially every request was challenged.` : ''} Data for those URLs reflects the WAF's challenge page, not the site. If this is your own site, allowlist the SEOAuditCrawler user agent and re-crawl.`, '#fee2e2');
    const rows = (bar.pages || []).slice(0, 10).map(p => [p.url, p.statusCode, p.vendor]);
    table(['Challenged URL', 'Status', 'WAF'], rows, [PW - 130, 50, 80]);
  }
  if (bpr.tested && bprDiffs.length > 0) {
    tip('ISSUE', `Cloaking check: ${bprDiffs.length} of ${bpr.testedCount} sampled page(s) served different content to "${bpr.comparedWith}" than to "${bpr.crawledAs}" (status, title, canonical or noindex). Verify this is intentional — inconsistent serving across user agents can hurt indexing and violates Google's cloaking guidelines when content differs for Googlebot.`, '#fee2e2');
    const rows = bprDiffs.slice(0, 10).map(r => [r.url, r.diffs.map(d => d.field).join(', ')]);
    table(['URL', 'Differs in'], rows, [PW - 140, 140]);
  } else if (bpr.tested && bpr.testedCount > 0) {
    tip('NOTE', `Cloaking check: ${bpr.testedCount} page(s) re-fetched as "${bpr.comparedWith}" matched what the crawl saw — no user-agent-based content differences detected.`, '#dcfce7');
  }

  // Marketing parameter loss on redirects (GA4 / ads attribution)
  const rpp = analysis.redirectParamsReport || {};
  const rppDrops = [...(rpp.activeDrops || []), ...(rpp.passiveDrops || [])];
  if (rppDrops.length > 0) {
    tip('ISSUE', `${rppDrops.length} redirect(s) drop marketing parameters (utm_*, gclid, fbclid, msclkid…). Campaign traffic through these redirects loses attribution — GA4 reports it as direct/none and ad-platform click IDs never reach the landing page. Configure redirects to preserve the query string.`, '#fee2e2');
    const rows = rppDrops.slice(0, 15).map(d => [d.url, (d.dropped || []).join(', ')]);
    table(['Redirecting URL', 'Dropped Parameters'], rows, [PW - 180, 180]);
  } else if (rpp.tested && rpp.testedCount > 0) {
    tip('NOTE', `Marketing parameters (utm_*, gclid, fbclid…) were preserved through redirects on all ${rpp.testedCount} tested URL(s).`, '#dcfce7');
  }
  ...
  if ((lnk.orphanCount || 0) > 0) {
    ...
    table(['Orphan Page URL'], (lnk.orphanPages || []).slice(0, 15).map(u => [u]), [PW]);
  }
  ...
  if (sm.found) {
    ...
    statRow([
      ...
      { label: 'Non-200 in Sitemap', value: sm.non200InSitemapCount || 0, color: (sm.non200InSitemapCount || 0) > 0 ? C.danger : C.success },
    ...
    if ((sm.non200InSitemapCount || 0) > 0) {
      tip('ISSUE', `${sm.non200InSitemapCount} URLs in the sitemap return non-200 status codes. Remove or fix these URLs.`, '#fee2e2');
      table(['URL', 'Status'], (sm.non200InSitemap || []).slice(0, 15).map(u => [u.url, u.statusCode]), [PW - 60, 60]);
    }
    if ((sm.crawledNotInSitemapCount || 0) > 0) {
      ...
      table(['URL (Crawled, Not in Sitemap)'], (sm.crawledNotInSitemap || []).slice(0, 15).map(u => [u]), [PW]);
    }
    if ((sm.inSitemapNotCrawledCount || 0) > 0) {
      tip('NOTE', `${sm.inSitemapNotCrawledCount} sitemap URLs were not reached during the crawl (orphan pages or crawl limit).`, '#dbeafe');
      table(['URL (In Sitemap, Not Crawled)'], (sm.inSitemapNotCrawled || []).slice(0, 15).map(u => [u]), [PW]);
    }
  } else {
  ...
  if (rppDrops.length > 0) recs.push({ p: 'High', t: `Fix ${rppDrops.length} redirect(s) that drop marketing parameters (utm_*, gclid…). Preserve the query string on redirects or attribution data is lost in GA4/Google Ads.` });
  if (bprDiffs.length > 0) recs.push({ p: 'High', t: `Review ${bprDiffs.length} page(s) serving different content per user agent (cloaking check). Unintentional differences can hurt indexing.` });
  if ((bar.challengedCount || 0) > 0) recs.push({ p: 'Medium', t: `Allowlist the crawler in your WAF (${Object.keys(bar.byVendor || {}).join(', ')}) — ${bar.challengedCount} request(s) were challenged, so parts of this report reflect the WAF, not the site.` });
