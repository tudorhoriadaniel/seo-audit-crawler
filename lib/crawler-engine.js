...
class CrawlerEngine {
  constructor(options = {}) {
    ...
    // Politeness delay (ms) between request STARTS, across all workers.
    // Shared-host firewalls (Infomaniak, Imunify360, CSF…) silently drop
    // concurrent bursts and temp-ban the IP; spacing requests out the way
    // Screaming Frog's speed limit does is what keeps the crawl alive there.
    this.requestDelay = Math.min(Math.max(parseInt(options.requestDelay) || 0, 0), 10000);
    this._nextRequestSlot = 0;
  ...
  /**
   * Reserve the next request slot and wait for it. Serialises request starts
   * across all concurrent workers so the site sees at most one new connection
   * per `requestDelay` ms, regardless of the concurrency setting.
   */
  async _throttle() {
    if (!this.requestDelay) return;
    const now = Date.now();
    const wait = Math.max(0, this._nextRequestSlot - now);
    this._nextRequestSlot = Math.max(now, this._nextRequestSlot) + this.requestDelay;
