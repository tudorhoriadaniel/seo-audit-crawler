...
class CrawlerEngine {
  ...
  // Diagnostic: fetch one URL with the exact same headers/cookies the crawler
  // uses, then report what we actually got back. Used by /api/debug-fetch to
  // see the server's real response to us (vs what a browser sees).
  async debugFetch(url) {
    ...
    // Every href on the page that looks like a /page/N/ pagination link.
    const pageLinks = [];
    $('a[href]').each((_, el) => {
      const h = $(el).attr('href');
      if (h && /\/page\/\d+\/?($|\?)/.test(h)) pageLinks.push(h);
    });
    // Common pager containers, so we can tell if the nav exists at all.
    const pagerSelectors = ['nav.pagination', '.pagination', '.page-numbers', '.nav-links', '.pager', '[class*="paginat"]'];
    const pagerFound = {};
    for (const sel of pagerSelectors) pagerFound[sel] = $(sel).length;
    return {
      requestedUrl: url,
      finalUrl: resp.request?.res?.responseUrl || url,
      status: resp.status,
      server: resp.headers['server'] || null,
      cfRay: resp.headers['cf-ray'] || null,
      cfCacheStatus: resp.headers['cf-cache-status'] || null,
      contentType: resp.headers['content-type'] || null,
      contentEncoding: resp.headers['content-encoding'] || null,
      setCookie: !!resp.headers['set-cookie'],
      htmlLength: html.length,
      titleTag: $('title').first().text().trim() || null,
      relNext,
      relPrev,
      totalAnchors: $('a[href]').length,
      pageLinkCount: pageLinks.length,
      pageLinks: [...new Set(pageLinks)].slice(0, 50),
      pagerContainers: pagerFound,
  ...
  async _crawlPage(item) {
    ...
    if (isHtml && html) {
      ...
      // Also follow rel="next" / rel="prev" pagination links. WordPress
      // (Yoast/RankMath) emits these in <head> for archive pagination, and
      // they chain through every page of a listing even when the visible
      // pager is truncated ("1 2 3 … 9") or rendered via JS. This is how
      // Screaming Frog reaches deep archive pages — it's a real declared
      // link, not a synthesized URL.
      for (const rel of [extracted.relNext, extracted.relPrev]) {
        if (!rel) continue;
        let abs = rel;
        try { abs = new URL(rel, url).href; } catch { /* keep as-is */ }
        const relKey = this._dedupeKey(abs);
        if (this._isSameOrigin(abs) && !this.visited.has(relKey) && !this._isExcludedPath(abs) && this.stats.totalUrls < this.maxPages * 2) {
          this.queue.push({ url: abs, depth: depth + 1, parent: url });
          this.stats.totalUrls++;
        }
      }
    }
